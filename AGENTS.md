...
| 活动计划 | [CMD-04 安全快速永久删除原子计划](docs/开发计划/原子开发计划-CMD-04安全快速永久删除.md) | 正在实施的目标根安全、永久删除、逐目标结果与真实 Windows 门禁 |
| 验收 | [测试与验收](docs/testing/测试与验收.md) | 实现及发布必须取得的验证证据 |

## 实现与文档同步

- 用户授权实现后，从活动计划中选择依赖已满足的推荐单元，不按前端、Rust、数据库横向拆成无法验收的半成品。
- `CMD-01` 至 `CMD-03` 已完成；用户已明确授权连续实施 `CMD-04`，当前按 `ATOMIC-CMD-04-001` 从 `CMD04-SAFETY-01` 开始。完成后必须停在 `CMD-05` 前等待用户检查和新授权。
...
| ATOMIC-CMD-04-001 | 原子开发 | [CMD-04 安全快速永久删除](docs/开发计划/原子开发计划-CMD-04安全快速永久删除.md) |
